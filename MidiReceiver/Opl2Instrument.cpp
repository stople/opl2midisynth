...
void Opl2Instrument::onNoteOn(byte channel, byte note, byte velocity) {
	...
	*/
	...
	_channelMap[i].midiChannel  = channel;
	_channelMap[i].midiNote     = note;
	_channelMap[i].midiVelocity = 1; //round(velocity / 127.0);
	_channelMap[i].op1Level     = round((63 - _opl2.getVolume(i, OPERATOR1)) / 63.0);
	_channelMap[i].op2Level     = round((63 - _opl2.getVolume(i, OPERATOR2)) / 63.0);
